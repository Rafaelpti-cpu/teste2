...
**The plinth that was tried and removed.** White paper on cream is a weak
contrast, so a dark `bg-surface-inverse` panel was put behind the tag on phones.
It was treating a symptom: the tag looked washed out because it was being drawn
at a third of the screen's resolution (see the pixel-ratio note above), not
because it lacked a ground. Once the resolution was fixed the panel stopped
earning its place, and the shop asked for the rose ground back. The canvas is
`alpha: true`, so whatever the page puts behind it shows through — on `lg` that
is the hero's rose glow, on a phone it is plain cream, since the glow stays up
with the heading at that width.
