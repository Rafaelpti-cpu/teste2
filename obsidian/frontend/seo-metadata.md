...
> [!warning] Do not "improve" the sitemap into a route handler
> It has been broken twice, both times in ways a local build could not show:
>
...
> | `app/sitemap.xml/route.ts` | ✅ | ✅ | **404** — Vercel resolves a path with a file extension against the static filesystem before reaching the function |
>
> `revalidate` on the metadata convention is the combination that survives.
> Verify any change to this file **against a deployment**, never against
> `next start` — that is precisely what hid both failures.
